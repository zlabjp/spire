...
+ All configuration for the SPIRE agent and server are done via Kubernetes
  configmaps. The container image for SPIRE contains only binaries.
+ The bootstrap cert and key from the SPIRE source tree is used. In a production
  deployment, this needs to be replaced with a proper secure config using short
...
This example is using the bootstrap cert and key. The bootstrap key
is configured via [server-secrets.yaml](server-secrets.yaml). As mentioned, in
a production environment this should be replaced with a more secure
configuration and key rotation.

The value for **bootstrap.key** is a base64 encoding of
[dummy_upstream_ca.key](https://github.com/spiffe/spire/blob/master/conf/server/dummy_upstream_ca.key).
For reference, this value was generated by running the following base64
command:
...
such as /run/spire/data, /run/spire/config, /run/spire/secrets, and
/run/k8s-certs which will be bound in during the next step when the server
container is deployed.

In addition to the SPIRE server configuration, the configmap contains the
bootstrap certificate
[dummy_upstream_ca.crt](https://github.com/spiffe/spire/blob/master/conf/server/dummy_upstream_ca.crt).
...
+ **spire-config** - this is a reference to the **spire-server** configmap from
  the previous step. This is mounted at /run/spire/config.
+ **spire-secrets** - this is a reference to the **spire-server** secrets from
  step B2 and mounted at /run/spire/secrets.
+ **spire-data** - this is a hostPath for the server's sqlite database and
  ...
  more secure and robust persistence mechanism. This is mounted to
  /run/spire/data.
+ **k8s-sa-cert** - this is the public key used for validating service accounts
  ...
  kube-apiserver. This will be mounted at /run/k8s-certs/sa.pub.
...
### C2. Create agent configmap

The configuration for the SPIRE agent is in the
[agent-configmap.yaml](agent-configmap.yaml) configmap. You'll note
/run/spire/config and /run/spire/sockets which we'll bind during the next step,
similar to the server but a slightly different set.

The configmap also contains a copy of the bootstrap cert passed to the agent
via the **trust_bundle_path** configuration.
...
+ **spire-config** - this is a reference to the **spire-agent** configmap from
  the previous step, mounted at /run/spire/config.
+ **spire-sockets** - this is a hostPath which will be shared with all other
  ...
  workloads use to communicate with the SPIRE agent API. This is mounted at
  /run/spire/sockets.

The daemonset configuration will configure a livenessProbe that test's the
agent's API socket (currently this is done in a less than ideal way, we are
working on a better healthcheck mechanism).
...
```
rpc error: code = Unavailable desc = all SubConns are in TransientFailure, latest connection error: connection error: desc = "transport: Error while dialing dial unix /run/spire/sockets/agent.sock: connect: no such file or directory"
```
